...
private:
    segmenter::DepthSegmenter initSegmenter() {
        ...
        node_handle_.param<int>("width", width, width);
        node_handle_.param<int>("height", height, height);
        ...
        segmenter::DepthSegmenter depth_segmenter(fx, fy, cx, cy, width, height, segmenter_config_, plane_config_);
    ...
    void imgCallback(const sensor_msgs::Image::ConstPtr& depth_msg) {
        ...
        ROS_DEBUG("Process %d images", frame_cnt++);
